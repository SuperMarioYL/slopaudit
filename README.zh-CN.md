...
<p align="center">
  <picture>
    ...
    <img src="./assets/atlas-light.svg" width="880" alt="SlopAudit 数据流：CLI 遍历并解析 JS/TS 仓库为 AST，运行三个纯函数 AST 检测器，把 finding 聚合成确定性的 SlopScore（0–100），再渲染终端报告、HTML 热力图与 SVG 徽章——全程离线。">
  </picture>
</p>

一条命令遍历仓库（`scan/walk.ts`），把每个 JS/TS 文件解析成 AST（`scan/parse.ts`，开启 `errorRecovery`，现代语法永不让扫描崩溃）。四个**纯函数检测器** —— `over_abstraction`、`generic_boilerplate`、`plausible_but_wrong` 与 `dead_parameter` —— 把 AST 变成加权的 `SlopFinding`，再由 `score/aggregate.ts` 归一化成唯一一个确定性的 **SlopScore（0–100）**。`report/` 层把同一个分数渲染成三种形态：chalk 终端热力图、自包含 HTML 报告、shields 风格 SVG 徽章。整条流水线静态且离线 —— 不调 LLM、不联网，同一个仓库 → 同一个分数。在 CI 里，同一道关卡还以一个打包好的 **GitHub Action** 形式发布，把分数写进任务摘要。
...
npx slopaudit .
 ...
 scan/parse.ts    @babel/parser → 每个文件一棵 AST
      ...
      │
      ▼
 detectors/       四个纯函数 AST 检测器 → SlopFinding[]
   ├─ overAbstraction.ts
...
当 SlopScore 达到或超过 `fail-on` 时任务**失败**，并且该次运行的 **Summary** 标签页会展示 band、文件计数，以及一张按排名列出的"最糟文件"表格 —— 无需任何行内标注配置。Action 还把 `score` 与 `band` 暴露为 step 输出：
...
> 这个 Action 只是同一道 CLI 关卡之上的一层薄复合包装 —— 依旧 100% 静态、离线、确定性。行内逐行 PR 标注仍在路线图上；v0.3.0 先发布 Action + 任务摘要报告。
...
- [ ] **行内 PR 标注：** 对最糟文件逐行写 review 评论（Action 先发布任务摘要报告）。
